...
interface ScriptCardProps {
  doc: GenerationDocument & { type: "generation" };
  isDeleting: boolean;
  onDeleteClick: () => void;
}
...
export default function ScriptCard({ doc, isDeleting, onDeleteClick }: ScriptCardProps) {
        ...
        <div className="flex justify-between items-start gap-4">
          ...
          <div className="flex items-center gap-1.5 font-sans">
            {doc.platforms?.map((p) => (
              <span
                key={p}
                className={`text-[9px] px-2 py-0.5 rounded font-extrabold uppercase tracking-wide ${
                  platformColors[p] || "bg-gray-100 text-gray-600"
                }`}
              >
                {platformNames[p] || p}
              </span>
